import { loadEch0Echoes, type Ech0NormalizedEcho, type Ech0ExtensionType } from '../lib/ech0-bits';
...
if (!remoteList || !state) throw new Error('bits remote root missing');
...
const render = async (items: Ech0NormalizedEcho[]) => {
...
if (!enabled) {
  ...
  try {
    ...
    try {
      const result = await loadEch0Echoes({ sourceUrl, pageSize, maxPages, signal: controller.signal });
      await render(result.items);
      window.dispatchEvent(new CustomEvent('bits:remote-loaded', { detail: { items: result.items.map(toIndexItem) } }));
      state.dataset.state = result.partial ? 'partial' : result.items.length ? 'ready' : 'empty';
      state.textContent = result.partial
        ? `已加载 ${result.items.length} 条远程絮语，部分结果未拉取`
        : result.items.length
          ? `已加载 ${result.items.length} 条远程絮语`
          : '暂无远程絮语，当前显示本地内容。';
    } finally {
